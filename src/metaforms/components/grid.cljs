...
(defn- key-scroll
  [config e]
  (if-let [key-cfg (get (-> config :key-scroll) (.-key e))]
    (scroll-to e (:direction key-cfg) (:page key-cfg))))
...
(defn- init-scrolling
  ...
  (let [container (r/dom-node scroller)]
    ...
    (.addEventListener container "keydown" (partial key-scroll config) false)))
...
(def default-config {:key-scroll {"ArrowUp"   {:direction -1 :page false}
                                  "ArrowDown" {:direction 1  :page false}
...
(defn- the-table
  ...
  (let [scroll-height   (:scroll-height config)
    ...
    (r/create-class {:component-did-mount    (fn [this] (init-scrolling this config))
